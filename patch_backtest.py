"""
补丁脚本：为 /ssq/backtest 添加 mode="all" 支持
1. 修改 ssq_backtest 函数签名和逻辑，支持 mode="all"
2. 新增 _run_backtest 内部函数
3. 新增 _compare_backtest_modes 对比函数
"""

import re

filepath = r"C:\Users\Administrator\stock-analysis-api\main.py"

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# === 1. 替换 ssq_backtest 函数定义和开头部分 ===
# 找到函数定义行
old_start = '''@app.get("/ssq/backtest", tags=["双色球历史数据"])
...
new_start = '''@app.get("/ssq/backtest", tags=["双色球历史数据"])
async def ssq_backtest(periods: int = 200, mode: str = "day_gan"):
    """
    双色球玄学维度回测验证
...
if old_start in content:
    ...
    print("❌ 未找到函数定义（old_start）")
    # 尝试找近似位置
    idx = content.find('async def ssq_backtest')
    if idx > 0:
        print(f"   找到函数定义位置: {idx}")
    else:
        print("   未找到函数定义")

# === 2. 在函数开头插入 mode="all" 处理逻辑 ===
# 在 "if not _SSQ_HISTORY:" 之前插入
insert_marker = '    if not _SSQ_HISTORY:'
insert_code = '''    # 如果mode="all"，并行跑三种模式
    if mode == "all":
        results_all = {}
...
if insert_marker in content:
    # 检查是否已插入
    if "_run_backtest" not in content[:content.find(insert_marker)]:
        content = content.replace(insert_marker, insert_code + insert_marker, 1)
        print("✅ 插入了 mode='all' 处理逻辑")
    else:
        print("⚠️  似乎已经插入过 mode='all' 逻辑")
else:
    print("❌ 未找到插入点（insert_marker）")

# === 3. 在文件末尾（before if __name__）添加辅助函数 ===
# 找到文件末尾合适位置（在 @app.get("/ssq/pick") 之前）
# 实际上我们应该在 ssq_backtest 函数结束后插入

# 找到 ssq_backtest 函数的 return 语句
return_marker = '    return {\n        "periods_tested": periods,\n        "mode": mode,\n        "formatted_backtest": formatted_backtest,\n        "details": results,\n    }'

# 检查是否已经有新函数
if "_run_backtest" not in content:
    # 在 ssq_backtest 函数结束后插入新函数
    # 找到函数结束位置
...
def _compare_backtest_modes(results_all, periods):
    ...
    for mode, result in results_all.items():
        ...
        mode_scores[mode] = {
            ...
            "score": red_lift_sum + blue_lift_sum * 0.5,  # 红球权重更高
        }
...
'''
            ...
            print("✅ 插入了 _run_backtest 和 _compare_backtest_modes 函数")
        else:
            print("❌ 未找到函数结束位置")
    else:
        print("⚠️  _run_backtest 函数已存在，跳过插入")

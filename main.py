...
def build_operation_summary(fields: dict, holdings: list = None) -> str:
    ...
    # V5.34.5: ADX逆势检测（信号与ADX趋势相反）
    adx_trend = str(fields.get("adx_trend", "")).lower()
    signal_lower = signal.lower()
    is_adx_counter = ("bear" in adx_trend and "buy" in signal_lower) or \
                     ("bull" in adx_trend and "sell" in signal_lower)
    adx_counter_line = f"\n🚨 ADX逆势：ADX显示{adx_trend}趋势，{signal}信号为逆势交易，强烈建议观望或极轻仓（<3%）" if is_adx_counter else ""
    ...
    if not holdings:
        # 无持仓 — 直接给一句判断
        if is_adx_counter:
            suggestion = "ADX逆势交易（趋势方向与信号相反），强烈建议观望，仅可考虑极轻仓试探"
        elif adx < 25:
...
━━━━━━━━━━━━━━━━━━
📭 当前未持有 {symbol}
💡 {suggestion}{ct_line}{adx_counter_line}
"""
...
def _batch_analyze_one(symbol: str, market: str, market_trend: dict):
    ...
    try:
        ...
        result = {
            ...
            # V5.35.5: signal_summary 在下面重新计算（确保与最终 signal 一致）
            "signal_summary": "",
        }
        ...
        # V5.35.5: 重新计算 signal_summary，确保与最终 signal 一致
        result["signal_summary"] = build_signal_summary(
            result["signal"],
            result.get("adx_trend", ""),
            result.get("rsi", 50)
        )
